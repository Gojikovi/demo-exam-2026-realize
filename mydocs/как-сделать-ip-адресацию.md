...
Шлюзом по умолчанию будет IP-адрес на интерфейсе HQ-RTR в сторону VLAN 100: 192.168.1.1

### HQ-CLI

Изменяем последний октет из расчитанной сети VLAN 200

Например: 192.168.1.34/28 (учитываем что IP-адрес будет получен по DHCP из диапазона 192.168.1.34 - 192.168.1.46)

Шлюз по умолчанию придёт по DHCP: 192.168.1.33
...
> [!NOTE]
> Если в условий будет написано "не менее N адресов", действуем следующим образом

Мы не вычитаем широковещательный адрес и адрес сети при расчёте через ipmeter
...
Тем самым, мы обеспечили минимальное количество адресов, которое должно быть в сети
